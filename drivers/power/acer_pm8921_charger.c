...
extern int last_calc_fcc;  //BMS FCC
extern int last_calc_pc;  //BMS PC
extern int last_calc_soc;  //BMS SoC
extern int last_calc_ruc;  //BMS RUC
...
void pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	/* iqnore this function when MHL prsent */
	if (is_mhl_charge)
		return;

#ifdef CONFIG_MACH_ACER_A9
	/* limit max charger current to 1100mA for A9 */
	if (mA > AC_CHG_MA)
		mA = AC_CHG_MA;
#endif
	...
	*/
	if (mA <= USB_WALL_THRESHOLD_MA)
		usb_target_ma = 0;
	/* set a current to usb_target_ma to start unplug_check_work */
...
int pm8921_set_usb_power_supply_type(enum power_supply_type type)
{
	...
	if (type <= POWER_SUPPLY_TYPE_USB_ACA) {
		...
		the_chip->usb_psy.type = type;
	} else if (type == POWER_SUPPLY_TYPE_AFC) {
